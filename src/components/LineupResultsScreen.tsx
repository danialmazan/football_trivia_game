...
export function LineupResultsScreen(props: LineupResultsScreenProps) {
  ...
  return (
    <main className="results-shell lineup-results-shell">
      ...
      {submitted && boards && <LeaderboardTabs mode="lineup-challenge" boards={boards} currentNickname={nickname} />}
      ...
      <section className="round-recap"><div className="section-heading"><div><span className="eyebrow">Teamsheet log</span><h2>Round by round</h2></div></div><div className="recap-table" role="table" aria-label="Lineup round results"><div className="recap-row recap-row--head" role="row"><span>Rnd</span><span>Match</span><span>Player</span><span>Misses</span><span>Pts</span></div>{game.results.map((result, index) => <div className="recap-row" role="row" key={`${result.matchId}-${index}`}><span>{String(index + 1).padStart(2, '0')}</span><strong>{result.matchLabel}</strong><span>{result.playerName}</span><span>{result.incorrectGuesses.length}</span><b>{result.points}</b></div>)}</div></section>
